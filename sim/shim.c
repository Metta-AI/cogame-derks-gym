// cogame-derks-gym wasm shim around the vendored Puffer MOBA sim.
//
// Compiled twice by sim/build_sim.sh:
//   - against build/src-patched  -> build/derk_sim.wasm           (production)
//   - against build/src-pristine -> build/derk_sim_pristine.wasm  (-DPRISTINE,
...
#include "loadout_common.h"  // derk_apply_loadout(): shared with the viewer

static MOBA env;

// The applied stat blocks loadout_digest() hashes: zero until a loadout
// is applied, so an un-drafted episode digests the all-zero table
// (and the viewer, which applies nothing, reproduces it).
static float g_applied[NUM_PLAYERS][DERK_LOADOUT_FIELDS];
...
// in ascending pid order. Touches no RNG, no map cell, no allocation —
// see sim/loadout_common.h for why that matters and for the field order.
// Returns 0 on success, -1 on a bad pid.
__attribute__((export_name("apply_loadout")))
int apply_loadout(int pid, float base_health, float base_mana,
                  float base_damage, int basic_attack_cd, float move_speed,
                  int hp_gain_per_level, int mana_gain_per_level,
                  int damage_gain_per_level) {
    float block[DERK_LOADOUT_FIELDS] = {
    ...
    if (derk_apply_loadout(&env, pid, block) != 0)
        return -1;
    for (int f = 0; f < DERK_LOADOUT_FIELDS; f++)
        g_applied[pid][f] = block[f];
    return 0;
}

// Stat read-back for tests and the draft-reveal cross-checks; `which`
// codes are the DERK_STAT_* list in sim/loadout_common.h.
__attribute__((export_name("hero_stat")))
float hero_stat(int pid, int which) {
    return derk_hero_stat(&env, pid, which);
}

// FNV-1a over the 10x8 applied float32 table (sim/loadout_common.h).
// Recorded in the replay header; the viewer must reproduce it after
// pushing the same table in through viewer_set_loadout.
__attribute__((export_name("loadout_digest")))
unsigned int loadout_digest(void) {
    return derk_loadout_digest(g_applied);
}
