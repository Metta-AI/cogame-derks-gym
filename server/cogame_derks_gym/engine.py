...
Runs one MOBA episode against per-seat async action sources. Per tick, in
this order:

1. house actions first, deterministically (``house_source``): the four
   in-process house heroes (pids 3, 4, 8, 9) in ASCENDING pid order.
   The order is load-bearing — the brain module has one shared ``rand()``
   stream and puffernet *samples* from the softmax, so a different call
   order is a different episode.
2. all six seats' actions gathered concurrently as ONE parallel batch
   under the config tick deadline; anything missing/late/malformed is
   NOOP-filled.
3. the full (10, 6) post-clamp matrix is written to the sim and stepped.
4. rewards accumulated, the replay hook called, and the tick's stat
   increases turned into replay events (see :mod:`.events`).

A seat can never crash the episode: any exception, timeout, or bad
payload from a source degrades to the no-op action for its hero.
...
The wall-clock budget is timed from ``start_monotonic`` when given, so
the server can start the clock at the DRAFT (the design note's 645 s =
draft 45 + play 600) rather than at the first tick.
...
# The four stats the replay event vocabulary is derived from, and their
# agent_stat `which` codes (sim/shim.c).
EVENT_STAT_CODES = {"level": 0, "kills": 1, "deaths": 2, "towers_killed": 3}
EVENT_STAT_NAMES = tuple(EVENT_STAT_CODES)
...
class ActionSource(Protocol):
    ...
    async def get_actions(
        ...
        """Actions for this seat's hero at ``tick``.

        ``obs`` is the (1, 510) uint8 slice for this seat's hero. Returns
        a (1, 6)-shaped nested sequence of action values, or None to play
        NOOP this tick.
        """
        ...
...
class EpisodeResult:
    ...
    events: tuple[dict, ...] = ()         # replay events (see .events)
...
class LockstepEngine:
    def __init__(self, sim, config: GameConfig,
        ...
        # Seat -> its single hero pid (obs, actions and rewards all index
        # heroes by pid rows). One hero per seat, fixed map.
        self._seat_pids = [defaults.pid_for_seat(s)
                           for s in range(config.num_seats)]
        self._seat_slices = [slice(pid, pid + 1) for pid in self._seat_pids]
        # The four house heroes, driven in-process (ascending pid order).
        self._house_source = house_source
        ...
        self._house_error_logged = False
    ...
    async def run(self) -> EpisodeResult:
        ...
        # The budget covers the draft too when the server passes its own
        # start time (see the module docstring).
        start = time.monotonic() if self._start_monotonic is None \
        ...
        try:
            while True:
                ...
                # House heroes first, in ascending pid order (see the
                # module docstring: the brain shares one rand() stream).
                house_actions = self._house_actions(obs)
                ...
                for pid, row in house_actions.items():
                    actions[pid] = row
                for seat, (reply, cause) in enumerate(replies):
                    sanitized = _sanitize(reply)
    ...
    def _house_actions(self, obs: np.ndarray) -> dict:
        """The four house heroes' actions, ascending pid order.

        Degrade-never-hang: a house source that raises (a missing brain
        wasm, a wasm trap) is logged once and its heroes play NOOP for
        the rest of the episode — the seats keep playing.
        """
        if self._house_source is None:
            return {}
        try:
            return self._house_source.actions(obs)
        except Exception as exc:
            if not self._house_error_logged:
                self._house_error_logged = True
                print(f"house heroes: source raised {type(exc).__name__}: "
                      f"{exc} (pids {defaults.HOUSE_HERO_PIDS} play NOOP)",
                      file=sys.stderr)
            self._house_source = None
            return {}

    def _observe_events(self, tick: int) -> None:
        """Turn this tick's stat increases into replay events.

        Reads agent_stat(pid, which) for the four event-bearing stats on
        all ten heroes (40 wasm calls per tick, ~microseconds) — the
        alternative, re-deriving events in the viewer, would need a
        ...
        try:
            stats = [
                {name: int(self._sim.agent_stat(pid, which))
                 for name, which in EVENT_STAT_CODES.items()}
    ...
    def _build_result(self, reward_sums: np.ndarray) -> EpisodeResult:
        ...
        for seat in range(cfg.num_seats):
            if winner is None:
                ...
                team = defaults.team_for_seat(seat)
        ...
        return EpisodeResult(
            ...
            events=() if self._event_log is None
            else tuple(self._event_log.events()),
        )
...
def _sanitize(reply) -> np.ndarray | None:
    """Validate one seat's reply into (1, 6) uint8, or None if malformed."""
    if reply is None:
        return None
    try:
        ...
        return None
    if arr.shape != (1, defaults.ACTIONS_PER_HERO):
        return None
