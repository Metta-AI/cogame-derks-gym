...
# `coworld build` pre-creates the parent; a bare CI run does not, and the
# `cd` below would then exit 1 on a fresh checkout (ecos, 2026-08-23).
...
image_tag="cogame-derks-gym-viewer-build:$$"
container_id=""
cleanup() {
