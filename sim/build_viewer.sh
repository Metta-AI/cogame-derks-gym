...
#
#   viewer/dist/derk_viewer.{js,wasm,data} + index.html
#       + derk_chrome.css + derk_chrome.js + derk_items.svg   (browser bundle,
...
VIEWER_EXPORTS=_viewer_load,_viewer_seek,_viewer_advance,_viewer_advance_frame,_viewer_render_phase,_viewer_tick,_viewer_total_ticks,_viewer_set_speed,_viewer_get_speed,_viewer_set_playing,_viewer_playing,_viewer_done,_viewer_winner,_viewer_state_digest,_viewer_set_loadout,_viewer_loadout_digest,_viewer_ancient_health,_viewer_agent_stat,_viewer_hero_positions,_viewer_set_camera,_viewer_camera,_malloc,_free
...
emcc -O2 -DMOBA_RENDER -DPLATFORM_WEB -DGRAPHICS_API_OPENGL_ES3 \
    ...
    -o viewer/dist/derk_viewer.js
cp viewer/index.html viewer/dist/index.html
# The appended game block's own files (see viewer/index.html): the shell
# links derk_chrome.css and derk_chrome.js, and derk_chrome.js <use>s the
# hand-authored item glyphs out of derk_items.svg. All three ship next to
# index.html in the bundle (and are in the Dockerfile COPY list).
cp viewer/derk_chrome.css viewer/derk_chrome.js viewer/derk_items.svg viewer/dist/
...
# CI) makes build/derk_sim.wasm the matching sim; absent (viewer-only
# dev build), the warning is simply disabled.
if [ -f build/derk_sim.wasm ]; then
    sim_sha="$(shasum -a 256 build/derk_sim.wasm | cut -d' ' -f1)"
    printf 'window.SIM_WASM_SHA256 = "%s";\n' "$sim_sha" \
        > viewer/dist/sim_sha.js
else
    echo "build/derk_sim.wasm absent: sim-sha mismatch warning disabled" >&2
