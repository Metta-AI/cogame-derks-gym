"""Baseline cogame-derks-gym player: ``python -m players.baseline_player``.

Runs the upstream pretrained policy (vendored moba_weights.bin) through
build/derk_brain.wasm — puffernet compiled to wasm, weights embedded —
hosted via wasmtime exactly like the sim host (module cache,
``_initialize`` call, memory-growth stub; see cogame_derks_gym.sim.MobaSim).
...
DEFAULT_BRAIN_WASM_PATH = REPO_ROOT / "build" / "derk_brain.wasm"
...
class MobaBrain:
    """wasmtime host for build/derk_brain.wasm (see sim/brain_shim.c)."""
