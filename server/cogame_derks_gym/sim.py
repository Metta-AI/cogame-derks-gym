...
DEFAULT_WASM_PATH = REPO_ROOT / "build" / "derk_sim.wasm"
PRISTINE_WASM_PATH = REPO_ROOT / "build" / "derk_sim_pristine.wasm"
...
# hero_stat() `which` codes, mirroring DERK_STAT_* in
# sim/loadout_common.h (0-7 are catalog.STAT_FIELDS in order).
HERO_STAT_CODES = {
    **{field: i for i, field in enumerate(catalog.STAT_FIELDS)},
    "max_health": 8, "max_mana": 9, "damage": 10,
    "health": 11, "mana": 12, "level": 13,
}
...
class MobaSim:
    ...
    def apply_loadout(self, pid: int, block: dict) -> None:
        """Write one hero's absolute post-clamp stat block into the sim.

        Called on the freshly initialised sim, after ``c_reset`` and
        before the first ``step()``, once per pid in ascending pid order
        (sim/loadout_common.h explains why re-spawning is forbidden
        there). ``block`` is keyed by
        :data:`cogame_derks_gym.catalog.STAT_FIELDS`.

        Argument validation at the wasm boundary follows the same
        discipline as ``set_actions``: raise on a missing field, a
        non-finite value, or a value outside the catalog's clamp range.
        The graceful degrade lives one layer up (the server substitutes
        the neutral loadout).
        """
        if not 0 <= pid < NUM_AGENTS:
            raise ValueError(f"pid must be 0..{NUM_AGENTS - 1}, got {pid}")
        args = []
        for field in catalog.STAT_FIELDS:
            if field not in block:
                raise ValueError(f"loadout block is missing {field!r}")
            value = float(block[field])
            if not math.isfinite(value):
                raise ValueError(f"{field} is not finite: {block[field]!r}")
            low, high = catalog.CLAMPS[field]
            if not float(low) <= value <= float(high):
                raise ValueError(
                    f"{field}={value} is outside the catalog clamp "
                    f"[{low}, {high}]")
            args.append(int(value) if field in catalog.INT_FIELDS else value)
        rc = self._exports["apply_loadout"](self._store, pid, *args)
        if rc != 0:
            raise RuntimeError(f"apply_loadout({pid}) failed ({rc})")

    def hero_stat(self, pid: int, which: int) -> float:
        """One hero's stat read-back; `which` codes are the DERK_STAT_*
        list in sim/loadout_common.h."""
        return float(self._exports["hero_stat"](self._store, pid, which))

    def loadout_digest(self) -> int:
        """u32 FNV-1a digest of the 10x8 applied loadout table. Recorded
        in the replay header; the viewer must reproduce it after pushing
        the same table in (viewer_set_loadout)."""
        return self._exports["loadout_digest"](self._store) & 0xFFFFFFFF
