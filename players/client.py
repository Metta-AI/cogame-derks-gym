"""Reusable async player harness for cogame-derks-gym websocket seats.

Speaks the server's lockstep wire protocol (see
``cogame_derks_gym.server``): one pre-match draft turn, then one JSON
text message per tick each way.
...
The draft turn is optional for a policy: a policy with no ``on_draft``
attribute simply never answers it, the server's deadline elapses and the
seat plays the neutral loadout. Messages with an unrecognised ``phase``
are ignored.
...
sequence of 6 ints per hero. It may additionally carry
``on_draft(observation) -> dict | None`` (sync or async) to answer the
...
async def _answer_draft(ws, policy, observation: dict) -> None:
    ...
    try:
        ...
        print(f"player: draft decision failed ({type(exc).__name__}: "
              ...
              f"the neutral loadout for this seat)", file=sys.stderr)
        return
    if picks is None:
        return
