...
Wire protocol. Before tick 0 there is exactly one draft turn (all six
seats asked in a single parallel batch under one shared deadline; see
:mod:`cogame_derks_gym.draft`), then the inherited per-tick loop:

    server -> player  {"phase": "draft", ...}           (once, pre-match)
    player -> server  {"phase": "draft", "picks": [{...}]}
    server -> player  {"phase": "draft_result", ...}    (informational)

    server -> player  {"tick": t, "obs": ["<base64 510B>"]}
    player -> server  {"tick": t, "actions": [[6 ints]]}
    server -> player  {"done": true, "result": {...}}   (episode end)

Messages with an unrecognised ``phase`` are ignored by both sides, and a
``{"tick": ...}`` reply arriving during the draft turn is ignored (it
does not consume the turn) — so an existing cogame-moba policy container
plays this game unmodified: it eats one draft timeout, gets the neutral
loadout, and then plays every tick normally.
...
Entry point: ``python -m cogame_derks_gym.server``. Binds ``COGAME_HOST``/
...
from . import catalog, defaults, draft, uris
...
<body>
<h1>cogame-derks-gym live viewer</h1>
...
<html>
<head><meta charset="utf-8"><title>cogame-derks-gym seat</title></head>
<body style="font-family: ui-monospace, monospace; margin: 2rem;">
<h1>cogame-derks-gym</h1>
...
class WsSeat:
    ...
    def __init__(self, slot: int, name: str, alias: str):
        self.slot = slot
        self.name = name
        # The in-game name. A policy sees only this; `name` is the real
        # player name and is spectator-side only.
        self.alias = alias
        ...
        # The single draft turn: at most one reply is consumed, later
        # draft messages are ignored. `{"tick": ...}` replies arriving
        # during the turn are counted here and do NOT consume it.
        self._draft_waiter: asyncio.Future | None = None
        self.draft_wrong_phase_count = 0
    ...
    async def get_draft(self, observation: dict):
        """Send this seat's draft observation and await its one reply.

        Returns ``(reply, cause)``: the decoded JSON object and None, or
        None with a draft.FALLBACK_CAUSES cause. The caller's shared
        deadline cancels the wait, which is the "timeout" case.
        """
        ws = self.ws
        if ws is None or ws.closed:
            return None, "disconnected"
        fut = asyncio.get_running_loop().create_future()
        self._draft_waiter = fut
        try:
            await ws.send_str(json.dumps(observation))
            return await fut
            ...
            self._draft_waiter = None

    async def send_draft_result(self, payload: dict) -> None:
        """Push the draft-reveal record set (no reply expected)."""
        ws = self.ws
        if ws is None or ws.closed:
            return
        try:
            await asyncio.wait_for(
                ws.send_str(json.dumps(payload)), DONE_SEND_TIMEOUT_SECONDS)
        except Exception:
            pass  # informational: never blocks the episode

    def deliver_raw(self, raw: str) -> None:
        """Route one raw client text frame.

        Frame-size and JSON validity are decided HERE for the draft turn,
        because an oversize frame must be dropped before the JSON parse
        (docs/PROTOCOL.md resolution order step 3).
        """
        waiter = self._draft_waiter
        if waiter is not None and not waiter.done():
            if len(raw.encode("utf-8", "ignore")) \
                    > catalog.MAX_DRAFT_FRAME_BYTES:
                waiter.set_result((None, "oversize"))
                return
            try:
                data = json.loads(raw)
            except json.JSONDecodeError:
                waiter.set_result((None, "wrong_shape"))
                return
            if not isinstance(data, dict):
                waiter.set_result((None, "wrong_shape"))
                return
            if data.get("phase") != "draft":
                # An unrecognised phase (or a per-tick reply arriving
                # early) is ignored and does NOT consume the draft turn.
                self.draft_wrong_phase_count += 1
                if self.draft_wrong_phase_count == 1:
                    print(f"seat {self.slot} ({self.alias}): non-draft "
                          f"message during the draft turn "
                          f"(phase={data.get('phase')!r}); ignored",
                          file=sys.stderr)
                return
            waiter.set_result((data, None))
            return
        try:
            data = json.loads(raw)
        except json.JSONDecodeError:
            return  # malformed: never crash the episode
        self.deliver(data)
    ...
    def fail_waiter(self) -> None:
        ...
        if waiter is not None:
            ...
            if not fut.done():
                fut.set_result(None)
        draft_waiter = self._draft_waiter
        if draft_waiter is not None and not draft_waiter.done():
            draft_waiter.set_result((None, "disconnected"))
...
class GameServer:
    def __init__(self, config: GameConfig, *,
        ...
        self.seats = [
            WsSeat(slot, player.name, defaults.SEAT_ALIASES[slot])
            for slot, player in enumerate(config.players)]
        # Phase, for the /global connect snapshot: draft -> play -> done.
        self.phase = "draft"
        # The ten draft-reveal records (None until the draft resolves).
        self.draft_records: list[dict] | None = None
    ...
    async def _handle_global(self, request: web.Request):
        ...
        snapshot = {
            ...
            "aliases": [s.alias for s in self.seats],
            "phase": self.phase,
            "max_ticks": self.config.max_ticks,
            "done": self.results_doc is not None,
        }
        if self.draft_records is not None:
            # Alias-only: the /global feed is a spectator surface, but a
            # policy could also read it, so it never carries real names.
            snapshot["loadouts"] = draft.spectator_records(self.draft_records)
    ...
    async def run_episode(self) -> EpisodeResult:
        ...
        # The engine's wall-clock budget is timed from HERE — the start of
        # the draft turn — not from the first tick (design note: 645 s =
        # draft 45 + play 600).
        ...
        try:
            ...
            # -- Phase B: the draft turn ---------------------------------
            if cfg.draft_enabled:
                records = await draft.run_draft(cfg, list(self.seats))
                draft.apply_to_sim(sim, records)
            else:
                # Puffer-fidelity mode: no draft turn and no
                # apply_loadout call at all, so the sim is byte-identical
                # to the upstream env. The records still describe it.
                print("draft disabled: every hero runs the neutral "
                      "loadout and no loadout is applied", file=sys.stderr)
                records = draft.neutral_records(cfg)
            self.draft_records = records
            writer.set_draft(records, self._loadout_digest(sim))
            await self._announce_draft(records)
            self.phase = "play"

            engine = LockstepEngine(
                sim, cfg, self.seats, on_tick=on_tick,
                on_seat_dead=self._on_seat_dead,
                house_source=self._house_source(),
                event_log=event_log,
                start_monotonic=episode_start)
        ...
        event_log.add_end(result.final_tick, result.end_reason)
        writer.set_events(event_log.events())
        writer.set_final_state_digest(self._state_digest(sim))
    ...
    def _house_source(self):
        """The four in-process house heroes, or None if the brain is
        unavailable (they then play NOOP — logged, never fatal)."""
        try:
            return HouseHeroes()
        except Exception as exc:
            print(f"house heroes unavailable ({type(exc).__name__}: {exc}); "
                  f"pids {defaults.HOUSE_HERO_PIDS} play NOOP",
                  file=sys.stderr)
            return None

    @staticmethod
    def _loadout_digest(sim) -> int:
        try:
            return int(sim.loadout_digest())
        except Exception:
            return 0

    @staticmethod
    def _state_digest(sim) -> int:
        try:
            return int(sim.state_digest())
        except Exception:
            return 0

    async def _announce_draft(self, records: list[dict]) -> None:
        """Push the draft-reveal record set to every seat and to /global.

        Alias-only on both surfaces: real player names are spectator-side
        (results.names and the replay header's config.players).
        """
        payload = {"phase": "draft_result",
                   "loadouts": draft.spectator_records(records)}
        await asyncio.gather(
            *(seat.send_draft_result(payload) for seat in self.seats),
            return_exceptions=True)
        self._broadcast_global(payload)
        for rec in records:
            if rec["source"] != "seat":
                continue
            print(f"draft {rec['alias']} ({rec['role']}): "
                  f"{rec['picks']['arm']} / {rec['picks']['tail']} / "
                  f"{rec['picks']['misc']} "
                  f"[{rec['fallback_cause']}, {rec['decision_ms']}ms]",
                  file=sys.stderr)
    ...
    def _results_doc(self, result: EpisodeResult) -> dict:
        ...
        return {
            ...
            "team": [
                defaults.TEAM_NAMES[defaults.team_for_seat(seat)]
            ...
            # the ten draft-reveal records (real names included: results
            # are a spectator artifact) and one flag per seat
            "draft": self.draft_records if self.draft_records is not None
            else draft.neutral_records(cfg),
            "draft_fallbacks": self._draft_fallbacks(),
        }
    ...
    def _draft_fallbacks(self) -> list[bool]:
        """One boolean per seat: did the server substitute the neutral
        loadout for it?"""
        records = self.draft_records or []
        by_seat = {rec["seat"]: rec for rec in records
                   if rec["seat"] is not None}
        return [bool(by_seat[seat]["fallback"]) if seat in by_seat else False
...
<head>
<meta charset="utf-8">
<title>cogame-derks-gym replay</title>
<style>
  body { font-family: ui-monospace, monospace; margin: 2rem; }
...
async function load() {
  ...
  if (magic !== "DERK") throw new Error("bad magic: " + magic);
  const version = buf[4];
...
def make_replay_app(replay_bytes: bytes,
    ...
    async def handle_replay_client(request: web.Request):
        if have_bundle:
            # index references its assets relatively (derk_viewer.js next
    ...
    if have_bundle:
        # bundle assets (derk_viewer.{js,wasm,data}) live next to index
...
async def async_main() -> int:
    ...
    if load_replay_uri:
        ...
        await site.start()
        print(f"cogame-derks-gym replay mode on {host}:{port} "
    ...
    site = web.TCPSite(runner, host, port)
    await site.start()
    print(f"cogame-derks-gym serving on {host}:{port} "
          f"({config.num_seats} seats, one hero each; house heroes "
          f"{defaults.HOUSE_HERO_PIDS}; "
          f"draft_enabled={config.draft_enabled})", file=sys.stderr)
